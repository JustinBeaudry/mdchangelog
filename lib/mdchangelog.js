...
// dependency check
var re = /(update|upgrade) (\w+)\/(\w+) to (.*) from (.*)/;
...
function MDChangelog(opts) {
    ...
    async.eachLimit(issuesList, 50, function(item, asyncCb) {
      chaps.get({
        ...
        if (res.body.title) {
          ...
          var match = issues[item.key].title.match(re);
          // look for any dependency links
          if(match) {
            dependentIssues[item.key] = {
              key: item.key,
  ...
  function buildLog() {
    ...
    changelogData = {
      ...
      dependents: dependentIssues
    };

    if (opts['orphan-issues'] === false) {
      changelogData.orphanIssues = [];
    }

    if (opts.prologue === false) {
      changelogData.prologue = false;
    }
  }

  function writeLog(cb) {
    if (opts.json) {
      return cb(null, changelogData);
    }
    console.log('=====================');
    console.log(dependentIssues);

    if (milestonesList.length || orphanIssues.length) {
      var tpl = fs.readFileSync(path.join(__dirname, 'log.ejs'), 'utf8');
      var output = ejs.render(tpl, changelogData) + existingChangelog;
  ...
  return function generate(cb) {
    ...
    async.series([
      ...
      if (err) {
        ...
        var fns = [];
        Object.keys(dependentIssues).forEach(function(key){
          fns.push(function(cb){
            ...
            mktemp.createDir('/tmp/mdchangelog-XXXXXXX', function(err, path) {
              ...
              var cmd = 'git clone git@github.com:' + dependentIssues[key].repo + '.git ' + path + '/' + dependentIssues[key].repo;
              exec(cmd, {}, function(err, stdout, stderr) {
                // console.log('err', err);
                // console.log('stdout', stdout);
                console.log('stderr', stderr);

                var a = new MDChangelog(deap.merge({
                  cwd: path + '/' + dependentIssues[key].repo,
                  json: true
                }, opts));
                a(function(err, res){

                  var inLog;
                  res.milestonesList.forEach(function(milestone){
                    if(milestone.title === dependentIssues[key].to) {
                      inLog = true;
                    }
                    if(inLog) {

                      dependentIssues[key].changelog.push(milestone);
                    }
                    if(milestone.title === dependentIssues[key].from) {
                      inLog = false;
                    }
                  });

                  console.log('xxxxxxxx', dependentIssues);
                  cb(null, changelogData);
                });
        ...
        if(fns.length) {
          async.parallel(fns, function(err, res){
            ...
            res.forEach(function(list){
              Object.keys(list).forEach(function(issue) {
                console.log(issue);
                // console.log(list[issue]);
                // dependentIssues[issue] = list[issue];
              });
            ...
            // NOTE:
            // need to redo approach entiry to be able to run multiple
            // mdchangelogs entirely separately. There needs to be an mdchangelog runner
            // that is controlling all of the flow.
