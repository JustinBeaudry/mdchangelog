...
// dependency check
var re = /(update|upgrade) (\w+)\/(\w+) to (.*) from (.*)/i;
...
function MDChangelog(opts) {
  ...
  function fetchIssues(cb) {
    ...
    if (issuesList.length && !opts.stdout) {
      ...
      var bar = new ProgressBar('[' + repo + '] fetching :current/:total issues [:bar] :percent :etas', {
        complete: '=',
        incomplete: ' ',
        width: 20,
        total: issuesList.length
      });
      process.stdout.write('[' + repo + '] fetching 0/' + issuesList.length + ' issues ');
    }
    async.eachLimit(issuesList, 50, function(item, asyncCb) {
      request({
        method: 'GET',
        uri: 'https://api.github.com/repos/' + item.repo + '/issues/' + item.number,
        timeout: opts.timeout || 10000,
        json: true,
        headers: {
          Authorization: 'token ' + process.env.MDCHANGELOG_TOKEN,
          'User-Agent': 'mdchangelog'
        }
        ...
        if (res.body.title) {
          ...
          if (opts.dependents) {
            ...
            // if same-org restriction is on make sure that we check for an org match
            if (match && (!opts['same-org'] || (opts['same-org'] && org === match[2]))) {
          ...
          if (res.body.milestone) {
            if (!opts['only-closed-milestones'] || res.body.milestone.closed_at !== null) {
  ...
  function buildLog() {
    ...
    var startMoment = moment.unix(startCommit.date.timestamp).utcOffset(0);
    var endMoment = moment.unix(endCommit.date.timestamp).utcOffset(0);
